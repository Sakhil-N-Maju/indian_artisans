...
export class ReviewsRatingsSystem {
   ...
   */
  private async updateReviewSummary(targetId: string, targetType: Review['type']): Promise<ReviewSummary> {
    ...
    const verifiedReviewsPercentage = totalReviews > 0
    ...
    if (reviewsWithCriteria.length > 0) {
      const criteriaKeys = ['quality', 'value', 'shipping', 'communication', 'accuracy', 'authenticity'] as const;
   ...
   */
  async getQuestions(targetId: string, targetType: ReviewQuestion['targetType']): Promise<ReviewQuestion[]> {
