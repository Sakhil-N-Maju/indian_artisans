...
export interface PaymentIntent {
  ...
  status: 'created' | 'requires-payment-method' | 'requires-confirmation' | 'processing' | 'succeeded' | 'cancelled';
...
export interface PaymentDispute {
  ...
  reason: string;
  category: 'fraudulent' | 'duplicate' | 'product-not-received' | 'product-unacceptable' | 'subscription-cancelled' | 'general';
...
export class PaymentGatewayIntegrationSystem {
  ...
  capturePayment(transactionId: string, amount?: number): PaymentTransaction {
    ...
    transaction.status = captureAmount === transaction.amount.total ? 'captured' : 'partially-refunded';
    transaction.timeline.captured = new Date();
    transaction.authorization.capturedAmount = (transaction.authorization.capturedAmount || 0) + captureAmount;
    transaction.authorization.remainingAmount = (transaction.authorization.remainingAmount || 0) - captureAmount;
  ...
  resolveDispute(disputeId: string, outcome: 'won' | 'lost' | 'accepted', notes?: string): PaymentDispute {
    const dispute = this.disputes.get(disputeId);
    if (!dispute) throw new Error('Dispute not found');
  ...
  private verifyWebhookSignature(gateway: PaymentGateway, payload: string, signature: string): boolean {
    const config = this.configurations.get(gateway);
  ...
  getPaymentAnalytics(period: { start: Date; end: Date }): PaymentAnalytics {
    ...
    const successful = transactionsInPeriod.filter(t => t.status === 'completed' || t.status === 'captured');
    ...
    const analytics: PaymentAnalytics = {
      ...
      overview: {
        ...
        averageTransactionValue: transactionsInPeriod.reduce((sum, t) => sum + t.amount.total, 0) / transactionsInPeriod.length || 0,
      },
      byGateway: [],
      byPaymentMethod: [],
      byCountry: [],
    ...
    analytics.refunds.rate = (analytics.refunds.total / analytics.overview.totalTransactions) * 100 || 0;
  ...
  getSavedPaymentMethods(customerId: string): SavedPaymentMethod[] {
    return Array.from(this.savedPaymentMethods.values())
      .filter(pm => pm.customerId === customerId && !pm.isExpired);
  }

  getPendingWebhooks(): PaymentWebhook[] {
    return Array.from(this.webhooks.values())
      .filter(w => w.processing.status === 'pending' || w.processing.status === 'processing');
  }
}
