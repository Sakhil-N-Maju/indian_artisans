...
class AIProductCreationService {
  ...
  private async ruleBasedCreation(
    ...
    return {
      ...
      product: {
        ...
        description: voiceInput?.transcription || 'Beautiful handcrafted item made by skilled artisans',
  ...
  private mergeProductData(
    ...
    return {
      title: voiceData.title || (imageAnalysis ? this.generateTitle(imageAnalysis) : 'Handcrafted Product'),
      description: voiceData.description || imageAnalysis?.description || 'Beautiful handcrafted item',
  ...
  suggestVariants(product: ProductListingData): ProductVariant[] {
    ...
    if (product.category.toLowerCase().includes('textile') || 
