...
export function WorkshopCalendar({ sessions, onBookSession, workshopTitle }: WorkshopCalendarProps) {
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-7 gap-2">
            {days.map((date, index) => {
              ...
              const hasAvailableSessions = daySessions.some(s => s.status === 'available' || s.status === 'limited')
              ...
              return (
                <button
                  ...
                  className={`
                    relative aspect-square rounded-lg border-2 p-2 text-sm transition-all
                    ${isSelected ? 'border-orange-500 bg-orange-50' : 'border-transparent'}
                    ${!isCurrentMonth(date) ? 'text-muted-foreground/40' : ''}
                    ${isToday(date) ? 'bg-blue-50 border-blue-300' : ''}
                    ${hasAvailableSessions && isCurrentMonth(date) ? 'hover:border-orange-300 hover:bg-orange-50/50 cursor-pointer' : ''}
                    ${!hasAvailableSessions || !isCurrentMonth(date) ? 'cursor-not-allowed opacity-50' : ''}
                  `}
                >
                      ...
                      {daySessions.slice(0, 3).map((session, i) => (
                        <div
                          ...
                          className={`w-1.5 h-1.5 rounded-full ${
                            ...
                            session.status === 'limited' ? 'bg-yellow-500' :
                            session.status === 'full' ? 'bg-red-500' :
                            'bg-gray-400'
                          }`}
                        />
                      ))}
