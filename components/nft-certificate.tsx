  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-amber-600 p-6 text-white">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-white/90 text-sm">Blockchain-Verified NFT Certificate</p>
          </div>
        ...
        <div className="mt-6 bg-white/10 backdrop-blur-sm rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-white/70 mb-1">Certificate Number</p>
              <p className="text-lg font-mono font-bold">{record.certificateNumber}</p>
            </div>
            <div className="text-right">
              ...
              <Badge variant="outline" className={getStatusColor(record.verificationStatus)}>
                {record.verificationStatus === 'verified' && <CheckCircle className="h-3 w-3 mr-1" />}
      ...
      <CardContent className="p-6 space-y-6">
        ...
        {/* Product Details */}
        <div>
          <h3 className="font-semibold text-xl mb-4">{record.productName}</h3>
        ...
        {record.provenance.length > 0 && (
          <>
            <Separator />
            <div>
              <h4 className="font-semibold mb-3">Provenance Trail</h4>
              <div className="space-y-3">
                {record.provenance.map((stage, index) => (
                  <div key={index} className="flex items-start gap-3">
                    <div className={`mt-1 w-2 h-2 rounded-full ${stage.verified ? 'bg-green-500' : 'bg-gray-300'}`} />
